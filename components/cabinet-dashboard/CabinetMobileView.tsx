...
import { CabinetTransactionExtended, cabinetAccounts, calculateBalanceSummary } from '@/lib/mock-data/cabinet-dashboard-data';
import CabinetFilters from './CabinetFilters';
import CabinetBalanceWidgets, { CabinetAccount, BalanceSummary } from './CabinetBalanceWidgets';
import TransactionDetailsSheet from './TransactionDetailsSheet';
import CabinetTransactionModal from './CabinetTransactionModal';
import { fadeInUp, bottomSheet } from '@/lib/animations/variants';
import { cn, formatTransactionAmount } from '@/lib/utils';

interface CabinetMobileViewProps {
  transactions: CabinetTransactionExtended[];
  selectedTransactions: Set<string>;
  onToggleSelection: (id: string) => void;
  onOpenJustificatif: (transaction: CabinetTransactionExtended) => void;
  searchTerm: string;
...
export default function BankMobileView({
  ...
  selectedAccounts,
  onAccountToggle
}: CabinetMobileViewProps) {
  const [selectedTransaction, setSelectedTransaction] = useState<CabinetTransactionExtended | null>(null);
  const [showDetailsSheet, setShowDetailsSheet] = useState(false);
  const [modalTransaction, setModalTransaction] = useState<CabinetTransactionExtended | null>(null);

  const handleTransactionClick = (transaction: CabinetTransactionExtended) => {
    setSelectedTransaction(transaction);
    setShowDetailsSheet(true);
  };

  const handleSwipeAction = (transaction: CabinetTransactionExtended, action: 'reconcile' | 'view' | 'link') => {
  ...
  const mobileAccounts = cabinetAccounts.slice(0, 3);
